export const EXPENSE_CATEGORIES = [
  { id: 'materials', name: 'Materials', icon: 'Package' },
  { id: 'labor', name: 'Labor', icon: 'Users' },
  { id: 'equipment', name: 'Equipment', icon: 'Truck' },
  { id: 'permits', name: 'Permits', icon: 'FileCheck' },
  { id: 'utilities', name: 'Utilities', icon: 'Zap' },
  { id: 'transportation', name: 'Transportation', icon: 'Car' },
  { id: 'tools', name: 'Tools', icon: 'Wrench' },
  { id: 'safety', name: 'Safety', icon: 'Shield' },
  { id: 'rental', name: 'Rental', icon: 'Home' },
  { id: 'insurance', name: 'Insurance', icon: 'FileText' },
  { id: 'miscellaneous', name: 'Miscellaneous', icon: 'MoreHorizontal' },
  { id: 'subcontractor', name: 'Subcontractor', icon: 'Building' },
];

export const PROJECT_STATUSES = [
  { id: 'active', name: 'Active', color: 'bg-green-500' },
  { id: 'payment-due', name: 'Outstanding', color: 'bg-orange-500' },
  { id: 'completed', name: 'Completed', color: 'bg-blue-500' },
  { id: 'on-hold', name: 'On Hold', color: 'bg-yellow-500' },
];

export const SUBCONTRACTOR_TRADES = ['Civil Work', 'Plumbing', 'Tiling', 'Electrical', 'Iron Work', 'Joinery Work'];

export const PERIOD_FILTERS = [
  { id: 'week', name: 'This Week' },
  { id: 'month', name: 'This Month' },
  { id: 'year', name: 'This Year' },
];